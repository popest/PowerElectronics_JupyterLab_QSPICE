...
def parse_and_generate_script(input_filename, output_filename):
    ...
    function_signature = f"def {function_name}({', '.join(param_args)}):\n"
    ...
    for_loop_code += f"{indent}return results"
    ...
    processing_lines = [
        ...
        f'{indent}run_output = subprocess.run([exe_qspice64, "{input_filename}"])\n\n'
        f"{indent}# Run postprocess measurement\n"
        f'{indent}run_output = subprocess.run([exe_qpost, "{input_filename}", "-o", "results.txt"])\n\n'
